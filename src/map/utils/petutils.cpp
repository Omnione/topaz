...
namespace petutils
{
    ...
    ************************************************************************/
    ...
    void SpawnMinion(CBattleEntity* PMaster, uint32 ModelID, bool spawningFromZone)
    {
        ...
        LoadMinion(PMaster, ModelID, spawningFromZone);
    ...
    */
    ...
    void LoadPet(CBattleEntity* PMaster, uint32 PetID, bool spawningFromZone)
    {
        ...
        */
        ...
        else if (PetID == PETID_MINION)
        {
            petType = PETTYPE_MINION;
        }
    ...
    {
        PETID id = PETID_MINION;
        Pet_t* PPetData = new Pet_t();

        PPetData = *std::find_if(g_PPetList.begin(), g_PPetList.end(), [id](Pet_t* t) { return t->PetID == id; });

        if (PMaster->objtype == TYPE_PC)
        {
            ((CCharEntity*)PMaster)->petZoningInfo.petID = id;
        }

        PETTYPE petType = PETTYPE_MINION;
        ...
        PPet->look = PPetData->look;
        PPet->name = PMaster->name + "'s " + PPetData->name;
        PPet->m_name_prefix = PPetData->name_prefix;
        PPet->m_Family = PPetData->m_Family;
        PPet->m_MobSkillList = PPetData->m_MobSkillList;
        PPet->SetMJob(PPetData->mJob);
